  ...
  <div style="flex:1; min-width:220px;">
    ...
    <div style="margin-top:28px;">
      <div style="display:flex;flex-wrap:wrap;gap:14px;justify-content:center;">
        <!-- Streamlit Get Started and navigation buttons -->
        <div>
          {st.button('Get Started', key='get_started', help='Jump to city input')}
        </div>
        <div>
          {st.button('Forecast', key='nav_forecast')}
        </div>
        <div>
          {st.button('Insights', key='nav_insights')}
        </div>
        <div>
          {st.button('Explain', key='nav_explain')}
        </div>
        <div>
          {st.button('Health', key='nav_health')}
        </div>
        <div>
          {st.button('Policy', key='nav_policy')}
        </div>
        <div>
          {st.button('Community', key='nav_community')}
        </div>
      </div>
    </div>
<style>
