...
with col_hero_left:
    st.markdown('<div class="title">🌬️ Mframapa AI</div>', unsafe_allow_html=True)
    st.markdown('<div class="desc">Welcome to your intelligent air quality companion. Get real-time forecasts, health guidance, and actionable insights for your city and beyond.</div>', unsafe_allow_html=True)
    st.markdown('<div style="display:flex; flex-wrap:wrap; gap:14px; justify-content:center; margin-top:28px;">', unsafe_allow_html=True)
    btn_cols = st.columns(7)
    with btn_cols[0]:
        st.markdown('<a href="#search" class="quick-nav-btn">Get Started</a>', unsafe_allow_html=True)
    with btn_cols[1]:
        nav_forecast = st.button('Forecast', key='nav_forecast')
    with btn_cols[2]:
        nav_insights = st.button('Insights', key='nav_insights')
    with btn_cols[3]:
        nav_explain = st.button('Explain', key='nav_explain')
    with btn_cols[4]:
        nav_health = st.button('Health', key='nav_health')
    with btn_cols[5]:
        nav_policy = st.button('Policy', key='nav_policy')
    with btn_cols[6]:
        nav_community = st.button('Community', key='nav_community')
    st.markdown('</div>', unsafe_allow_html=True)
with col_hero_right:
    st.markdown('<img src="https://img.icons8.com/ios-filled/120/66d9ef/cloud.png" alt="cloud" style="opacity:0.9;max-width:100%;height:auto;" />', unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)
...
if nav_policy:
    st.switch_page("pages/4_Policy.py")
if nav_community:
...
with col_center:
  city_input = st.text_input("", placeholder="Type a city (e.g., Accra, Lagos, Los Angeles)", label_visibility="collapsed")
  search_btn = st.button("Search City", key="home_search", type="primary", use_container_width=True)
...
<div class="footer">
  <div class="social">
    ...
    <a href="https://x.com/yawoseii" target="_blank" title="X"><span>𝕏</span></a>
