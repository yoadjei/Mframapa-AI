...
with st.container():
    ...
    city_suggestions = ["Los Angeles, USA", "New York, USA", "Accra, Ghana", "Denver, USA", 
                       "Toronto, Canada", "Mexico City, Mexico", "Lagos, Nigeria"]
    
    col1, col2 = st.columns([3, 1])
    with col1:
        city_input = st.selectbox(
            "Enter or select your city",
            options=[""] + city_suggestions,
            placeholder="e.g., Los Angeles, Accra",
            help="Type or select a city to get started with air quality forecasts."
        )
    ...
    if lookup_button and city_input:
        with st.spinner(f"Looking up coordinates for {city_input}..."):
            ...
            if coordinates:
                ...
                st.info("📈 Your city is set! Visit the **Forecast** page for air quality predictions.")
