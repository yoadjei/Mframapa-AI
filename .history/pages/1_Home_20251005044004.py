...
def get_weather_data(city):
    ...
    try:
        ...
        if err.response.status_code == 401:
             return None, "Authentication error. Please check your OpenWeatherMap API key."
...
if st.session_state.view == 'main':
    ...
    with cols[0]:
        ...
        <div class="card feature-card">
            ...
            <p style="color: #6c757d;">Utilizing high-resolution data from NASA's TEMPO and MERRA-2 for global coverage.</p>
        </div>
        """, unsafe_allow_html=True)
    with cols[1]:
        st.markdown("""
        <div class="card feature-card">
            ...
            <p style="color: #6c757d;">Advanced XGBoost models predict PM2.5, O₃, and NO₂ up to 48 hours in advance.</p>
        </div>
        """, unsafe_allow_html=True)
    with cols[2]:
        st.markdown("""
        <div class="card feature-card">
            ...
            <p style="color: #6c757d;">Get personalized, activity-based guidance to protect your health from air pollution.</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    # Ensure we don't try to create more columns than we have data for
    forecast_days = list(daily_forecasts.items())
    num_cols = min(len(forecast_days), 5) # Display up to 5 days
