...
export function MobileShell({ children, canInstall, onInstall }) {
  ...
  const desktopTabs = ["home", "core", "search", "notifications", "activity", "profile", "settings"];
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/90 px-4 py-3 backdrop-blur dark:border-slate-800 dark:bg-slate-950/90">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          <div>
            <p className="text-sm font-semibold tracking-tight text-emerald-600">Mframapa v2</p>
        ...
        <div className="mx-auto mt-3 hidden max-w-7xl gap-2 md:flex">
          {desktopTabs.map((tab) => (
            <button
              key={tab}
              type="button"
              onClick={() => dispatch({ type: "SET_ACTIVE_SCREEN", payload: tab })}
            ...
            >
              {tab}
              {tab === "notifications" && unreadCount > 0 ? ` (${unreadCount > 9 ? "9+" : unreadCount})` : ""}
            </button>
          ))}
        </div>
      </header>
